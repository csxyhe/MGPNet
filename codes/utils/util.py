...
def processImg(img, img_multiple_of):
    h, w = img.shape[2], img.shape[3]
    if h % img_multiple_of == 0:
        pad_h = 0
    else:
        pad_h = img_multiple_of - h % img_multiple_of
    if w % img_multiple_of == 0:
        pad_w = 0
    else:
        pad_w = img_multiple_of - w % img_multiple_of
    img = F.pad(img, pad=(0, pad_w, 0, pad_h), mode='reflect')
    if pad_h == 0:
        pad_h = None
    if pad_w == 0:
        pad_w = None
